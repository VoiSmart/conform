defmodule IntegrationTest do
  ...
  test "effective configuration" do
    ...
    {:ok, conf} =
      Path.join(["test", "fixtures", "test_app", "test.conf"]) |> Conform.Conf.from_file()

    schema =
      Path.join(["test", "fixtures", "test_app", "test.schema.exs"]) |> Conform.Schema.load!()
  ...
  test "for the complex data types" do
    ...
    expected = [
      my_app: [
        ...
        complex_list: [buzz: [age: 25, type: :person], fido: [type: :dog]],
        some_val: :foo,
        some_val2: 2.5,
  ...
  test "test for the custom data type" do
    ...
    expected = [
      log: [console_file: "/var/log/console.log", error_file: "/var/log/error.log", syslog: :on],
      logger: [format: "$time $metadata[$level] $levelpad$message\n"],
      myapp: [
        {Custom.Enum, :prod},
        {Some.Module, [val: :foo]},
