defmodule Conform.ReleasePlugin do
  ...
  def before_assembly(%{profile: %{overlays: overlays} = profile} = release) do
    include_pre_start? = if_distillery(:lt, "1.2.0", do: true, else: false)

    pre_or_post_configure =
      if_distillery(:lt, "1.5.0", do: "pre_configure", else: "post_configure")

    post_configure_src =
      Path.join(["#{:code.priv_dir(:conform)}", "bin", "#{pre_or_post_configure}.sh"])
    ...
    conform_overlays =
      if include_pre_start? do
        [
          {:copy, post_configure_src,
           "releases/<%= release_version %>/hooks/pre_start.d/00_conform_pre_configure.sh"},
          {:copy, post_configure_src,
           "releases/<%= release_version %>/hooks/#{pre_or_post_configure}.d/00_conform_#{
             pre_or_post_configure
           }.sh"},
          {:copy, pre_upgrade_src,
           "releases/<%= release_version %>/hooks/pre_upgrade.d/00_conform_pre_upgrade.sh"},
          {:copy, post_upgrade_src,
           "releases/<%= release_version %>/hooks/post_upgrade.d/00_conform_post_upgrade.sh"},
  ...
  defp add_archive(conform_overlays, release, schema_src) do
    ...
    case result do
      ...
      {_, zip_path, _} ->
        [
          {:copy, "#{zip_path}", "releases/<%= release_version %>/<%= release_name %>.schema.ez"}
          | conform_overlays
        ]
    end
  end
  ...
  defp add_conf(conform_overlays, _release, conf_src) do
    case File.exists?(conf_src) do
      true ->
        [
          {:copy, conf_src, "releases/<%= release_version %>/<%= release_name %>.conf"}
          | conform_overlays
        ]
  ...
  defp generate_umbrella_schema(release) do
    schemas =
      Enum.reduce(umbrella_apps_paths(), [], fn {name, path}, acc ->
        schema =
          path
  ...
  defp generate_umbrella_conf(release) do
    conf_files =
      Enum.reduce(umbrella_apps_paths(), [], fn {name, path}, acc ->
        conf_path =
          case File.exists?(Path.join(path, "config/#{name}.#{Mix.env()}.conf")) do
  ...
  defp get_conf_path(release) do
    case releasing_umbrella?(release.name) do
      true ->
        generate_umbrella_conf(release)
