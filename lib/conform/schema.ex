defmodule Conform.Schema do
  ...
  defmodule SchemaError do
    ...
    defexception message:
                   "Invalid schema. Should be a keyword list with at least the :mappings key defined"
  end
  ...
  defp from(quoted, path) when is_list(quoted) do
    ...
    schema =
      case Keyword.get(quoted, :import) do
        nil ->
          schema
    ...
    schema =
      case Keyword.get(quoted, :mappings) do
        nil ->
          raise SchemaError, message: "Schema must contain at least one mapping!"
    ...
    schema =
      case Keyword.get(quoted, :transforms) do
        nil ->
          schema
    ...
    schema =
      case Keyword.get(quoted, :validators) do
        nil ->
          %{schema | :validators => global_validators}
          ...
          Enum.map(extends, fn
            e when is_atom(e) ->
              case get_extends_schema(e, path) do
                nil ->
                  Conform.Logger.warn(
                    "Schema extends #{e}, but the schema for #{e} was not found."
                  )
            ...
            e ->
              Conform.Logger.warn(
                "Invalid extends value: #{e}. Only application names as atoms are permitted."
              )
  ...
  defp to_list(%Conform.Schema.Validator{name: name, definition: v}),
    do: {String.to_atom(name), v}

  defp to_list(%Conform.Schema.Transform{path: path, definition: nil, transform: t}),
    do: {String.to_atom(path), t}

  defp to_list(%Conform.Schema.Transform{path: path, definition: t}),
    do: {String.to_atom(path), t}
  ...
  defp to_mapping(key, setting, value) do
    case Keyword.keyword?(value) do
      ...
      false ->
        ...
        Conform.Schema.Mapping.from_quoted(
          {:"#{setting_name}",
           [
  ...
  defp get_extends_schema(app_name, src_schema_path) do
          ...
          case :code.lib_dir(app_name) do
            ...
            path when is_list(path) ->
              ...
              case File.exists?(path <> ".ez") do
                true ->
                  Path.join([path <> ".ez", "#{app_name}", "config", "#{app_name}.schema.exs"])

                false ->
                  Path.join([path, "config", "#{app_name}.schema.exs"])
              end
          end
    ...
    schema_path =
      case schema_path == nil || File.exists?(schema_path) == false do
        true ->
              ...
              case :erl_prim_loader.list_dir('#{archive_path}') do
                ...
                {:ok, apps} ->
                  case '#{app_name}' in apps do
                    true ->
                      Path.join([archive_path, "#{app_name}", "config", "#{app_name}.schema.exs"])
    ...
    case schema_path do
      ...
      schema_path when is_binary(schema_path) ->
        case File.exists?(schema_path) do
          true ->
            {schema_path, File.read!(schema_path)}
  ...
  defp load_archive(archive_path) do
    case File.exists?(archive_path) do
      true ->
        ...
        apps =
          Enum.map(zip_files, fn {:zip_file, path, _, _, _, _} ->
            path = to_string(path)
