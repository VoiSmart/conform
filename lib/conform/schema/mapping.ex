defmodule Conform.Schema.Mapping do
    ...
    case Keyword.keyword?(mapping) do
      false ->
        raise Conform.Schema.SchemaError,
          message: "Invalid mapping for #{name}: `#{inspect(mapping)}`."
  ...
  defp do_from([{:to, to} | rest], mapping) when is_binary(to),
    do: do_from(rest, %{mapping | :to => to})

  defp do_from([{:datatype, dt} | rest], mapping), do: do_from(rest, %{mapping | :datatype => dt})

  defp do_from([{:default, default} | rest], mapping),
    do: do_from(rest, %{mapping | :default => default})

  defp do_from([{:env_var, env_var} | rest], mapping),
    do: do_from(rest, %{mapping | :env_var => env_var})

  defp do_from([{:doc, doc} | rest], mapping) when is_binary(doc),
    do: do_from(rest, %{mapping | :doc => doc})

  defp do_from([{:see, see} | rest], mapping) when is_binary(see),
    do: do_from(rest, %{mapping | :see => see})

  defp do_from([{:hidden, h} | rest], mapping) when is_boolean(h),
    do: do_from(rest, %{mapping | :hidden => h})

  defp do_from([{:commented, c} | rest], mapping) when is_boolean(c),
    do: do_from(rest, %{mapping | :commented => c})

  defp do_from([{:include_default, default} | rest], mapping) when is_binary(default),
    do: do_from(rest, %{mapping | :include_default => default})

  defp do_from([{:validators, vs} | rest], mapping) when is_list(vs),
    do: do_from(rest, %{mapping | :validators => vs})
