defmodule Conform.Translate do
  ...
  def to_conf(%Schema{mappings: mappings}) do
    ...
    mappings =
      Enum.filter(mappings, fn
        %Mapping{hidden: false} -> true
        _ -> false
      end)
  ...
  def to_config(%Schema{} = schema, config, conf_table_id) do
    ...
    config
    # Merge the conf over config.exs/sys.config terms
    |> merge(conf)
  end
  ...
  defp apply_schema(conf_table_id, %Schema{} = schema) do
    try do
      ...
      err ->
        Conform.Logger.error(
          "Error thrown when constructing configuration: #{Macro.to_string(err)}"
        )
    end
  end
          ...
          case mapping.env_var do
            ...
            var ->
              case System.get_env(var) do
                ...
                val ->
                  parse_datatype(datatype, [val], mapping)
              end
          end
  ...
  defp convert_complex_types(
         [%Mapping{name: key, datatype: complex} = mapping | rest],
         table,
         mappings
       )
        ...
        case mapping.env_var do
          ...
          var ->
            case System.get_env(var) do
              ...
              val ->
                {stripped, parse_datatype(mapping.datatype, [val], mapping)}
            end
        end
      end)

    children =
      Enum.map(selected, fn {name, _} -> List.first(strip_prefix(name, prefix)) end)
      |> Enum.uniq()

    selected =
      Enum.reduce(children, selected, fn child, acc ->
        defaults =
          Enum.reduce(child_mappings, [], fn
            {[], _default}, acc2 ->
              acc2
    ...
    transformed =
      case transform do
        ...
        _ ->
          ...
          Conform.Logger.error(
            "Invalid transform for #{problem_key}. Must be a function of arity 1"
          )
      end
  ...
  defp parse_datatype([list: types], values, %Mapping{name: name} = mapping)
       when is_list(types) and length(types) > 1 do
    Enum.map(values, fn element ->
      res =
        Enum.reduce(types, element, fn
          _type, {:ok, _} = result ->
            result
      ...
      case res do
        ...
        _other ->
          raise TranslateError,
            message:
              "Invalid format for #{stringify_key(name)}: Expected one of: #{inspect(types)}"
      end
    end)
  end
  ...
  defp parse_datatype(type, value, %Mapping{name: name}) do
    raise TranslateError,
          "Invalid value for #{stringify_key(name)}: Expected value of type `#{inspect(type)}`, got #{
            inspect(value)
          }"
  end
  ...
  defp parse_datatype_single(:ip, value, %Mapping{name: name}) do
    case String.split("#{value}", ":", trim: true) do
      ...
      _ ->
        raise TranslateError,
          message: "Invalid IP format for #{stringify_key(name)}. Expected format: IP:PORT"
    end
  end
  ...
  defp parse_datatype_single([list: type], value, mapping) do
    raise TranslateError,
      message:
        "Invalid value for #{stringify_key(mapping.name)}: Expected list of `#{inspect(type)}`, got: #{
          inspect(value)
        }"
  end
  ...
  defp parse_datatype_single(tuple, value, mapping) when is_tuple(tuple) and is_tuple(value) do
    ...
    cond do
      ...
      :else ->
        ...
        raise TranslateError,
          message: "Invalid format for #{stringify_key(name)}. Expected #{inspect(tuple)}"
    end
  end
  ...
  defp write_datatype(:ip, value, setting) do
    case value do
      ...
      _ ->
        raise TranslateError,
          message: "Invalid IP address format for #{setting}. Expected format: {IP, PORT}"
    end
  end
  ...
  defp write_datatype([list: list_types], value, setting)
    ...
    |> Enum.map(fn element ->
      ...
      cond do
        ...
        :else ->
          raise TranslateError,
            message: "Invalid datatype for #{setting}. Expected one of: #{inspect(list_types)}"
      end
    end)
    |> Enum.join(", ")
  end
  ...
  defp is_custom_type?(datatype) do
    ...
    if mod do
      ...
      mod =
        case String.match?(<<first::utf8>>, ~r/[A-Z]/) do
