defmodule Conform do
  ...
  defp process(switches) when is_list(switches) do
    ...
    case {conf, schema} do
      {nil, _} ->
        Logger.warn("--conf is required")
        process(:help)

      {_, nil} ->
        Logger.warn("--schema is required")
        process(:help)
  ...
  defp process(%Options{} = options) do
    ...
    final =
      case Conform.Conf.from_file(options.conf) do
    ...
    Logger.success(
      "Generated #{options.write_to |> Path.basename()} in #{options.write_to |> Path.dirname()}"
    )
  end
end
