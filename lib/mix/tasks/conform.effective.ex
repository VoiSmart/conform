defmodule Mix.Tasks.Conform.Effective do
  ...
  def run(argv) do
    ...
    cond do
      Mix.Project.umbrella?() && args.options.type == :app ->
        ...
        dep =
          Enum.find(Mix.Dep.Umbrella.loaded(), fn
            %Mix.Dep{app: ^app} -> true
            _ -> false
          end)
  ...
  defp do_run(args) do
    ...
    conf =
      if File.exists?(conf_path) do
        ...
        Conform.Logger.debug(
          "No .conf file found - defaults from schema and Mix config will be used"
        )
    ...
    try do
      ...
      Protocol.UndefinedError ->
        Conform.Logger.error(
          "The value for some key in #{path} doesn't implement the Access protocol!"
        )
    end
  end
  ...
  defp parse_args(argv) do
    ...
    mutually_exclusive_opts =
      Enum.filter([app: app, key: key, schema: schema], fn {_, val} -> val != nil end)
